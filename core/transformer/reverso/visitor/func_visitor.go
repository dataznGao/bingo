		...
		if can {
			for _, stmt := range decl.Body.List {
				if ifStmt, ok := stmt.(*ast.IfStmt); ok {
					visitor := &ReversoIfVisitor{lp: v.lp, value: v.value, File: v.File}
					ast.Walk(visitor, ifStmt)
				} else if forStmt, ok := stmt.(*ast.ForStmt); ok {
					visitor := &ReversoForVisitor{lp: v.lp, value: v.value, File: v.File}
					ast.Walk(visitor, forStmt)
				} else if caseStmt, ok := stmt.(*ast.SwitchStmt); ok {
					visitor := &ReversoCaseVisitor{lp: v.lp, value: v.value, File: v.File}
